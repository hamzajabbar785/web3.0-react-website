...
function Products() {
  ...
  return (
    <PageSection
      name="products"
      title="Products"
      subtitle={`Plainly lamplight whether evermore threat tell, we or scarce bird spoken he whether on, sinking thy again sent what eyes tinkled his explore these. Within melancholy sad bird not respiterespite...`}
    >
      <div className="grid lg:grid-cols-2 gap-12 text-black">
        {products.map(({ id, icon, title, subtitle }) => {
          return (
            ...
            >
              <div className="flex items-center justify-center duration-300 group-hover:scale-110 cursor-pointer">
                <div className="bg-black rounded-full p-5 m-4">{icon}</div>
              </div>

              <h1 className="text-3xl lg:text-5xl capitalize my-8">{title}</h1>
